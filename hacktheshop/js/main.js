...
/* ═══════════════════════════════════
   CART
   ═══════════════════════════════════ */
let cart = JSON.parse(localStorage.getItem('hts_cart') || '[]');

function syncCart() {
  localStorage.setItem('hts_cart', JSON.stringify(cart));
...
function renderCart() {
  ...
  const total = cart.reduce((s, i) => s + parseFloat((i.price||'').replace(/[^0-9.]/g,'')||0), 0);
  el.innerHTML = cart.map((item, i) => `
    <div class="cart-item">
      <div>
        <span>${item.name}</span>
        ${item.price ? `<span style="color:#a78bfa;font-size:12px;display:block;">${item.price}</span>` : ''}
      </div>
      <button onclick="removeFromCart(${i})" style="background:transparent;border:1px solid #7c3aed;color:#f472b6;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;font-family:inherit;">✕</button>
    </div>
  `).join('') + (total > 0 ? `<div style="padding:12px;border-top:1px solid #2d2d40;color:#a78bfa;font-weight:bold;">Total: $${total.toFixed(2)}</div>` : '');
}

/* ═══════════════════════════════════
   WISHLIST
   ═══════════════════════════════════ */
let wishlist = JSON.parse(localStorage.getItem('hts_wish') || '[]');
...
function toggleWish(btn) {
  ...
  localStorage.setItem('hts_wish', JSON.stringify(wishlist));
  renderWish();
}
...
function showToast(msg) {
  ...
  if (!t) {
    ...
    t.style.cssText = `
      ...
      background:rgba(19,19,31,0.95);border:1px solid #7c3aed;
      color:#e2d9f3;padding:12px 20px;border-radius:12px;
      font-family:'Courier New',monospace;font-size:0.85rem;
      z-index:9999;opacity:0;transition:opacity 0.3s;
      max-width:280px;box-shadow:0 0 20px rgba(124,58,237,0.3);
    `;
...
function toggleCSMenu(e) {
  ...
  const ch = document.getElementById('csChevron');
  if (ch) {
...
function openProductModal(name) {
  ...
  document.getElementById('modalAddCart').onclick   = () => addToCart(name, p.price);
...
function loadModalComments(productName) {
  ...
  fetch(`php/comments.php?action=get&product=${encodeURIComponent(productName)}`)
    ...
    .then(comments => {
      ...
      list.innerHTML = '<p class="empty-msg">Reviews unavailable.</p>';
    });
}
...
/* ═══════════════════════════════════
   PRODUCTS DATA (shared reference)
   ═══════════════════════════════════ */
const PRODUCTS_DATA = {
  ...
  'AirPods Pro 2':      { price:'$249',  img:'https://images.unsplash.com/photo-1606220945770-b5b6c2c55bf1?w=400&q=80',  desc:'Active Noise Cancellation, 30hr battery life.' },
